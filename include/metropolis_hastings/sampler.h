...
private:
  ...
  const int thinning_factor;

public:
  const size_t n_max;
  Sampler(Model &m, Eigen::VectorXd init_sample, size_t total_steps,
          const int thinning = 0)
      : rng(seed), target_model(m), proposer(m.dim, rng),
        current_sample(init_sample), dist(0.0, 1.0), thinning_factor(thinning),
        n_max(total_steps) {
  ...
  Eigen::MatrixXd get_history() { return history; }
};
